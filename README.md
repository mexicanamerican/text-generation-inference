...
docker run --gpus all,nvidia.runtime--shm-size 1g -p 8080:80 -v $volume:/data ghcr.io/huggingface/text-generation-inference:1.0.0 --model-id $model
```
...
print(client.generate_blog("What is Deep Learning?", max_new_tokens=20).generated_text)

text = ""
for response in client.generate_text_stream("What is Deep Learning?", max_new_tokens=20):
